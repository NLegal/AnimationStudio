...
if ($SkipDownload) {
    ...
    New-Item -ItemType Directory -Force -Path (Split-Path -Parent $CkptDest) | Out-Null
    # Use .NET WebClient for a progress bar (Invoke-WebRequest is slow on PS5).
    $wc = New-Object System.Net.WebClient
    try {
        $wc.DownloadFile($CkptUrl, $CkptDest)
    } finally {
        $wc.Dispose()
    }
    if (-not (Test-Path $CkptDest)) { Die "Model download failed." }
}
