#!/usr/bin/env python3
"""generate_face_lock.py — Face Lock: Expression generation for Lily Bunny.

Executes the second stage of the Progressive Locking Pipeline (D-05/D-06):
Face Lock produces a complete expression library that locks the character's
facial identity across 32 expressions.
...
Usage:
    python scripts/generate_face_lock.py
"""
...
# Number of candidates per expression variant
CANDIDATES_PER_EXPRESSION = 60
SHORTLIST_SIZE = 12

# Output directory for approved expression images
UNIVERSE_DIR = Path("Universe/Characters/Lily Bunny/expressions")
...
async def main():
    print("=" * 70)
    print("  Face Lock — Lily Bunny Expression Library (32 expressions)")
    print("=" * 70)
    print()

    # -- Precondition: ComfyUI must be running --
    batch_id = f"face_lock_{int(time.time())}"

    if not check_comfyui():
        # Allow script-only mode: create the script, note error, don't exit
        print(f"\n  Batch ID: {batch_id}")
        print("  Generated 0 / 32 expressions (ComfyUI not running)")
    ...
    try:
        ...
        if front_refs:
            ...
            print("  ⚠ No approved front reference found. Expressions will be generated")
            print("    without identity scoring reference (D-14 thresholds may not apply).")
    except Exception as exc:
        print(f"  ⚠ Could not load reference sheet: {exc}")
        print("    Continuing without identity reference — scoring will use defaults.")

    # -- Get the merged expression list at runtime (from PromptBuilder) --
    print("\n[4/6] Loading expression list from PromptBuilder...")
    expressions = sorted(prompt_builder._known_expressions())
    print(f"  ✓ {len(expressions)} expressions loaded")
    print(f"    {', '.join(expressions[:8])}... ({len(expressions) - 8} more)")

    # -- Generate expressions sequentially --
    print(f"\n[5/6] Generating expression candidates...")
    print(f"  Batch ID: {batch_id}")
    print(f"  Candidates per expression: {CANDIDATES_PER_EXPRESSION}")
    print(f"  Shortlist size per expression: {SHORTLIST_SIZE}")
    print(f"  Estimated total: {len(expressions) * CANDIDATES_PER_EXPRESSION} images")
    print()
    ...
    for idx, expression_name in enumerate(expressions):
        count = CANDIDATES_PER_EXPRESSION
        shortlist_size = SHORTLIST_SIZE

        print(f"  {'─' * 54}")
        print(f"  Expression {idx + 1}/{len(expressions)}: '{expression_name}' — "
              f"{count} candidates, shortlist top {shortlist_size}")
        print(f"  {'─' * 54}")

        # Build prompt for this expression
        positive, negative = prompt_builder.build(
            character=character,
            asset_type="expression",
            variant=expression_name,
        )
        ...
        for i, seed in enumerate(seeds):
            ...
            try:
                output = backend.generate(gen_input, asset_type="expression")
                if output.images:
        ...
        if not generated_images:
            print(f"  ✗ No images generated for '{expression_name}' — skipping")
            results[expression_name] = {"generated": 0, "scored": 0, "shortlisted": 0, "error": "no images"}
            continue
        ...
        for img_idx, img in enumerate(generated_images):
            ...
            try:
                ...
                asset.id = f"temp_{expression_name}_{img_idx}"
        ...
        if scored_count > 0:
            ...
            print(f"  ✗ No images survived scoring for '{expression_name}'")
            results[expression_name] = {"generated": len(generated_images), "scored": 0, "shortlisted": 0, "error": "no scored"}
            continue
        ...
        print(f"  ✓ {expr_shortlisted} assets shortlisted for human review")
        total_generated += len(generated_images)
        total_shortlisted += expr_shortlisted

        results[expression_name] = {
            "generated": len(generated_images),
            "scored": scored_count,
            "shortlisted": expr_shortlisted,
        }
    ...
    print(f"  Expressions with candidates: {success_count}/{len(expressions)}")
    if error_count:
        print(f"  Expressions with errors: {error_count}")
        for name, r in results.items():
            if r.get("error"):
                print(f"    ✗ {name}: {r['error']}")
    print()

    for expr_name, r in sorted(results.items()):
        err = f" — ERROR: {r.get('error', '')}" if r.get("error") else ""
        print(f"  {expr_name:25s}: {r['generated']:3d} generated, {r['scored']:3d} scored, "
    ...
    print()
    print(f"  To review expression candidates:")
    print(f"    1. Open http://localhost:8000 in your browser")
    print(f"    2. Navigate to:")
    print(f"       http://localhost:8000/review/{character_id}?asset_type=expression&batch=true&grid=4x4")
    print(f"    3. For each of the {len(expressions)} expressions:")
    print(f"       - Review shortlisted candidates")
    print(f"       - Compare against the front reference sheet (left panel)")
    print(f"       - Click 'Approve' or 'Approve & Promote' to lock a winner")
    print(f"    4. Approved assets will transition: shortlisted → approved → production")
    print()
    print(f"  ⚠ D-14 Approval thresholds for expressions: 92–97% identity similarity")
    print(f"  ⚠ D-15 Approval zones: ≥95% auto-pass, 90–95% normal review, <90% reject")
    print()
